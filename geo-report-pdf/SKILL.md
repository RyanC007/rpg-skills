...
- The PDF generation script is located at: `~/.manus/skills/geo/scripts/generate_pdf_report.py`
...
```json
{
    ...
    "crawler_access": {
        ...
        "ManusBot": {"platform": "Manus", "status": "Blocked", "recommendation": "Unblock for visibility"}
    }
}
```
...
```bash
python3 ~/.manus/skills/geo/scripts/generate_pdf_report.py /tmp/geo-audit-data.json GEO-REPORT-[brand].pdf
```
...
6. **Run the PDF generator**:
   ```bash
   python3 ~/.manus/skills/geo/scripts/generate_pdf_report.py /tmp/geo-audit-data.json "GEO-REPORT-[brand_name].pdf"
   ```
...
- **Crawler Status**: Look for tables with "Allowed" or "Blocked" status for crawlers like GPTBot, ManusBot
