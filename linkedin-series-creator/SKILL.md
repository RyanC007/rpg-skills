...
```bash
python /home/ubuntu/skills/linkedin-series-creator/scripts/scrape_linkedin_profile.py \
...
Repeat: true
Prompt: "Generate today's LinkedIn post draft using the linkedin-series-creator skill"
```

### Series Generation Workflow

When Ryan requests a post series (e.g., "build me a 5-post series on AI automation"):

**Step 1: Check Posting Cadence**

```bash
python /home/ubuntu/skills/linkedin-series-creator/scripts/track_posts.py \
...
Before generating, define the series arc:
- Series theme (e.g., "AI Automation for Founders")
- Number of posts (typically 3-7)
- Post sequence: each post should build on the last or cover a distinct angle
- Vary post types across the series (educational, story-based, tactical, thought leadership)
- Plan one infographic post per series where appropriate

**Step 3: Generate Each Post Draft**

```bash
python /home/ubuntu/skills/linkedin-series-creator/scripts/generate_linkedin_post.py \
  "./knowledge" \
  "./linkedin_data/ryan_profile_analysis.json" \
  "[specific topic for this post in the series]" \
  "[post_type]"
```
...
Run once per post in the series. The script outputs a JSON object with the generated post content.

**Step 4: Review and Format**

Review each generated post for:
...
- Flows logically as part of the series sequence

Use `templates/post_templates.md` as a reference for structure.

**Step 5: Deliver the Full Series to Ryan**

Send a message to the user with:
- Series title and theme
- All post drafts numbered and sequenced (Post 1 of N, Post 2 of N, etc.)
- Suggested posting schedule
- Optional: Infographic suggestion if applicable
- Call to action: "Review the series and let me know which posts to adjust before we schedule them."
